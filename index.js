...
slimbot &&
  slimbot
    .sendMessage(config.telegramUserID, `Trader started`, {
      parse_mode: "Markdown"
    })
    .catch(console.error);
...
function botReportTelegram(traderBot) {
  ...
  traderBot.on("priceUpdate", price => {
    ...
    upPct *= 100;
    let msg = `Target price for ${traderBot.asset} updated: ${price.toFixed(
      8
    )}. New target ${upPct.toFixed(2)}%`;
    slimbot
      .sendMessage(ID, msg, {
        parse_mode: "Markdown"
      })
  ...
  slimbot.on("message", msg => {
    ...
    switch (true) {
      ...
      case bot && action[0] === "/sell":
        ...
        slimbot.sendMessage(
          ID,
          `Sell message received! Selling ${bot.asset} for ${price}.`
        );
        bot.sell({
          price: price
        });
      ...
      default:
        slimbot.sendMessage(
          ID,
          `Action not recognized! Type /help for a list of commands.`
        );
        break;
    }
  });
...
async function startTrader(data = null, telegramAction = false) {
  ...
  if (!bot.isResuming && !data) {
    return;
  }
  if (!bot.isResuming && data && data.aidata.length) {
    // console.log(data)
    if (bot && bot.is_trading) {
      console.log(`Bot is trading!`);
      return;
    }
    const pair = data.aidata
      .sort((a, b) => {
    ...
    if (diff < 15) {
      //if signal is more than 15 minutes, wait for next
      let x = null;
